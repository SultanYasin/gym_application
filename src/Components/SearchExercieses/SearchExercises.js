...
const SearchExercieses = () => {
  ...
  useEffect(()=>{
    const fetchExercises = async () => { 
      const bodyPartsData = await fetchData("https://exercisedb.p.rapidapi.com/exercises",exerciesOptions);
      setBodyParts(['all' , ...bodyPartsData])
  }
    fetchExercises()
  },[])
  ...
  const handleSearch = async () => {
    if (search) {
      const exerciesesData = await fetchData(
        "https://exercisedb.p.rapidapi.com/exercises",
        exerciesOptions
      );
      const searchedExercises = exerciesesData.filter(
        (exercise) =>
